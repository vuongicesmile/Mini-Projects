...
function deQuyLevel($array, $parent, &$newString) {
    // điều kiện dừng
    if (count($array) > 0) {
        $newString.='<ul>';
      foreach ($array as $key => $value) {
        if ($value['parent'] == $parent) {
          ...
          $newString .= '<li>'.$value['name'].'</li>';
          unset($array[$key]);
          $newParent = $value['id'];
          deQuyLevel($array, $newParent, $newString);
          $newString.='</li>';
        }
      }
      $newString.='</ul>';
  ...
  deQuyLevel($menu,0,$newString);
  $newString = str_replace('<ul></ul>','',$newString);
//   $newString = str_replace('<li></ul>','<ul>',$newString);
//   $newString = str_replace('<li></ul>','</li></ul></li>',$newString);
